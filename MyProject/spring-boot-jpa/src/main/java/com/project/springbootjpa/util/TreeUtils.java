...
public class TreeUtils {
	...
	/**
	 * 生成树形节点 map包含3个字段 id 为节点主键 name 为节点名称 parentId 为上级节点主键
	 *
	 * @param dataList
	 * @return
	 */
	public static JSONObject genTree(List<HashMap<String, String>> dataList) {
		if (0 < dataList.size()) {
			...
			for (Iterator it = dataList.iterator(); it.hasNext();) {
				...
				node.id = Long.valueOf((String) dataRecord.get("id"));
				node.code = "" + dataRecord.get("code");
				node.name = "" + dataRecord.get("name");
				// node.parentId = "" + dataRecord.get("parentId");
				node.parentId = dataRecord.get("parentId") == null
						|| StringUtils.isBlank((String) dataRecord.get("parentId")) ? null
								: Long.valueOf((String) dataRecord.get("parentId"));
			...
			for (Iterator it = entrySet.iterator(); it.hasNext();) {
				...
				// StringUtils.isBlank(node.parentId)
	...
	/**
	 * 生成树形节点 默认包含4个字段 id 为节点主键 menuName 为节点名称 menuCodewei parentId 为上级节点主键
	 *
	 * @param dataList
	 * @return
	 */
			...
			for (TreeNode dataRecord : dataList) {
				if (nodeList.containsKey(dataRecord.getId())) {
					...
					if (nodeList.get(dataRecord.getId()) instanceof HashMap) {
						...
						node.put(((TreeNode) nodeList.get(dataRecord.getId())).getParentId(),
								nodeList.get(dataRecord.getId()));
			...
			for (Iterator it = entrySet.iterator(); it.hasNext();) {
				...
				if (record instanceof HashMap) {
					...
					for (Iterator nodeIt = nodeSet.iterator(); nodeIt.hasNext();) {
						...
						// StringUtils.isBlank(reCordNode.getParentId()) || null ==
						// nodeList.get(reCordNode.getParentId())
						if (null == nodeList.get(reCordNode.getParentId())) {
							root = reCordNode;
							treeNodes.add(root);
						} else {
				...
				// StringUtils.isBlank(node.getParentId())|| null ==
				// nodeList.get(node.getParentId())

				if (null == nodeList.get(node.getParentId())) {
					root = node;
					treeNodes.add(root);
				} else {
			...
			if (null != treeProp) {
				try {
					...
					if (TreeField.ID.equals(treeProp.value())) {
						// StringUtil.getString(f.get(dataList))
						node.setId((Long) f.get(dataList));
					} else if (TreeField.PARENTID.equals(treeProp.value())) {
						if (f.get(dataList) != null) {
							// StringUtil.getString(f.get(dataList))
							node.setParentId((Long) f.get(dataList));
						}
	...
	public static void main(String[] args) {
		...
		dataRecord9.put("id", "16");
		dataRecord9.put("code", "16");
		dataRecord9.put("name", "廊坊银行开发区支行");
		dataRecord9.put("parentId", "");

		HashMap dataRecord10 = new HashMap();
		dataRecord10.put("id", "1601");
		dataRecord10.put("code", "1601");
		dataRecord10.put("name", "廊坊银行总行");
		dataRecord10.put("parentId", "16");

		HashMap dataRecord11 = new HashMap();
		dataRecord11.put("id", "1602");
		dataRecord11.put("code", "1602");
		dataRecord11.put("name", "廊坊分行");
		dataRecord11.put("parentId", "16");

		HashMap dataRecord12 = new HashMap();
		dataRecord12.put("id", "160101");
		dataRecord12.put("code", "160101");
		dataRecord12.put("name", "廊坊银行金光道支行");
		dataRecord12.put("parentId", "1601");

		HashMap dataRecord13 = new HashMap();
		dataRecord13.put("id", "16010101");
		dataRecord13.put("code", "16010101");
		dataRecord13.put("name", "廊坊银行解放道支行");
		dataRecord13.put("parentId", "160101");

		HashMap dataRecord14 = new HashMap();
		dataRecord14.put("id", "16010102");
		dataRecord14.put("code", "16010102");
		dataRecord14.put("name", "廊坊银行三大街支行");
		dataRecord14.put("parentId", "160101");

		HashMap dataRecord15 = new HashMap();
		dataRecord15.put("id", "16010201");
		dataRecord15.put("code", "16010201");
		dataRecord15.put("name", "廊坊银行广阳道支行");
		dataRecord15.put("parentId", "1602");

		HashMap dataRecord16 = new HashMap();
		dataRecord16.put("id", "16010201");
		dataRecord16.put("code", "16010201");
		dataRecord16.put("name", "廊坊银行三大街支行");
		dataRecord16.put("parentId", "1602");
		...
		for (Object map : dataList) {
			...
			nodes.add(t);
			t.setId(Long.valueOf(((HashMap) map).get("id").toString()));
			if (((HashMap) map).get("parentId") != null
					&& StringUtils.isNotBlank(((HashMap) map).get("parentId").toString())) {
				t.setParentId(Long.valueOf(((HashMap) map).get("parentId").toString()));
			}
